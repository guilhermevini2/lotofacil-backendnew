...
def backtest(concursos, dezenas_pool, inicio=None, fim=None):
    ...
    total_pool = 0
    soma_pool_hits = 0
    dist_pool_hits = {}  # quantos concursos tiveram X das 15 dezenas dentro do pool
    ...
    for cs in cs_filtro:
        ...
        soma_pool_hits += pool_hits
        dist_pool_hits[pool_hits] = dist_pool_hits.get(pool_hits, 0) + 1
    ...
    return {
        ...
        # Metricas continuas -- muito mais informativas que pct_dentro_pool,
        # que exige as 15 dezenas dentro do pool (evento raro por definicao,
        # ~0.025% ao acaso para um pool de 18/25) e por isso costuma ficar em 0%.
        "media_pool_hits":     round(soma_pool_hits / n, 2) if n else 0,
        "media_pool_hits_pct": round(soma_pool_hits / n / DEZENAS_SORTEADAS * 100, 1) if n else 0,
        "dist_pool_hits":      dist_pool_hits,
        "faixas":           contagem,
        "detalhes":         detalhes,
    }
...
def resumo_backtest(resultado):
    ...
    linhas = [
        ...
        f"  Media de dezenas do sorteio dentro do pool: {resultado.get('media_pool_hits', 0)}/15 ({resultado.get('media_pool_hits_pct', 0)}%)",
        f"  Sorteios com as 15 dezenas dentro do pool : {resultado['total_pool']} ({resultado['pct_dentro_pool']}%) -- evento raro por definicao",
        "",
