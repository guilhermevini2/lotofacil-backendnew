"""
servidor_nuvem.py - LotofacilPro v4 — Versao para deploy na nuvem.

Diferencas do servidor.py local:
  - Sem Node.js (Puter.js via CDN no frontend)
  - Cache em memoria (nao usa disco - nuvem tem filesystem efemero)
  - CORS habilitado (acesso de qualquer dispositivo)
  - Gunicorn-ready (servidor de producao)
  - Sem matplotlib/openpyxl (reduz tamanho do deploy)
  - Endpoints identicos ao servidor.py local

Deploy gratuito:
  Railway : https://railway.app  (recomendado - 500h/mes gratis)
  Render  : https://render.com   (750h/mes gratis)
  Fly.io  : https://fly.io       (3 VMs gratis)
"""
...
# Garantir que os modulos do projeto sejam encontrados
...
_ia_estado = {
    "status": "idle", "progresso": 0,
    "etapa": "", "dados": None, "erro": None,
}
...
def _rodar_analise(pool_size: int = 18, modo: str = "basico"):
    try:
        ...
        _set(progresso=24, etapa="Pares e trios quentes...")
        ...
        _set(progresso=36, etapa="Scores hibridos...")
        ...
        res_auto = executar_ciclo_autoaprendizado(
            concursos, ml_probas=None, verbose=False
        )
        pesos_base = carregar_pesos()
        rows_h = ranking_hibrido(concursos, pesos_base, None, clasf)
        ...
        _set(progresso=65, etapa="AI Provider...")
        payload_ia = {
            ...
            "pool_atual": [r["dezena"] for r in rows_h[:18]],
        }

        resp_ia = {"pool_final": [], "estrategia": {}, "provider": "local"}
        if HAS_AI_PROVIDER:
            try:
        ...
        _set(progresso=72, etapa="Selecionando pool...")
        sel_ps = selecionar_pool(
            concursos, tamanho=pool_size,
            n_atrasadas=2, janela_recente=20, atraso_min=8,
        )
        pool_ia = resp_ia.get("pool_final", [])
        if pool_ia and len(pool_ia) == 18:
            pool_final = sorted(set(int(d) for d in pool_ia if 1 <= int(d) <= 25))[:18]
        else:
            pool_final = sel_ps["pool"][:18]
        ...
        resultado_pool_sel = {
            ...
            "atrasos_forcadas": {str(d): sel_ps["atrasos"][d]
                                  for d in sel_ps["forcadas"]},
            ...
            "n_moldura": val_pool["n_moldura"],
            "n_centro": val_pool["n_centro"],
            "checklist": {
        ...
        _set(progresso=82, etapa="Fechamento matematico...")
        val_mat = validator.validar_matriz()
        pool_18 = pool_final[:18]
        jogos_brutos = fech_mod.gerar_jogos(pool_18)

        _set(progresso=87, etapa="Filtros modo aviso...")
        jogos_finais, _ = filtrar_jogos(jogos_brutos, modo=MODO_AVISO)

        _set(progresso=90, etapa="Backtests...")
        bt_fixo = simulator.backtest(concursos, pool_18)
        bt_din = simulator.backtest_dinamico(
            concursos, tamanho_pool=18, janela_treino=60
        )
        ...
        resultado_ai["provider_explicacao"] = (
            resp_ia.get("estrategia", {}).get("explicacao", "")
        )
        ...
        if HAS_CHAT:
            get_chat().atualizar_contexto({
                ...
                "backtest": {
                    "pct_dentro_pool": bt_usar.get("pct_dentro_pool", 0),
        ...
        resultado = {
            ...
            "trios_quentes": [{"a": a, "b": b, "c": c, "count": cnt}
                               for a, b, c, cnt in trios[:15]],
            "pools": [],
            ...
            "hibrido": {
                ...
                "ranking": [
                    {"posicao": r["posicao"], "dezena": r["dezena"],
                     "score": r["score"], "tendencia": r["tendencia"],
                     "seta": r["seta"], "label_tend": r["label_tend"],
                     "freq_10": r["freq_10"], "freq_20": r["freq_20"],
                     "freq_50": r["freq_50"], "atraso": r["atraso"],
                     "pressao": r["pressao"],
                     "no_pool": r["dezena"] in set(pool_18)}
                    for r in rows_h
                ],
                "tendencias": [
                    {"dezena": c["dezena"], "tendencia": c["tendencia"],
                     "seta": c["seta"], "label": c["label"],
                     "freq_10": round(c["freq_10"] * 100, 1),
...
def _rodar_gerar_inteligente():
    try:
        _ia_set(status="carregando", progresso=5,
                etapa="Verificando analise base...")
        estado = _get()
        if estado["status"] != "pronto" or not estado["resultado"]:
            raise RuntimeError(
                "Execute a analise principal primeiro."
            )
        resultado_base = estado["resultado"]

        _ia_set(progresso=20, etapa="Preparando dados para a IA...")
        ...
        payload_ia = {
            ...
            "soma_media":    resultado_base.get(
                "estatisticas", {}).get("soma", {}).get("media", 0),
            "pesos_atuais":  hib.get("pesos", {}),
            "acumulou":      (resultado_base.get("acumulacao") or {}).get("alerta", False),
        }

        _ia_set(progresso=45, etapa="Consultando AI Provider...")
        resp_ia = {"pool_final": [], "estrategia": {}, "provider": "local"}
        if HAS_AI_PROVIDER:
            try:
                resp_ia = ai_analisar(payload_ia, verbose=False)
        ...
        _ia_set(progresso=72, etapa="Validando e gerando jogos...")
        pool_ia = resp_ia.get("pool_final", [])
        if pool_ia and len(pool_ia) >= 15:
            pool_ia = sorted(
                set(int(d) for d in pool_ia if 1 <= int(d) <= 25)
            )[:18]
        else:
        ...
        jogos_ia = (fech_mod.gerar_jogos(pool_ia)
                    ...
                    else resultado_base.get("jogos", []))
        if not pool_ia:
            pool_ia = resultado_base.get("dezenas_18", pool_ia)

        _ia_set(progresso=85, etapa="Pontuando jogos...")
        jogos_com_score = []
        soma_ideal = 200.0
        for i, jogo in enumerate(jogos_ia, 1):
            soma = sum(jogo)
            score = max(30.0, 100.0 - abs(soma - soma_ideal) / 2)
            pares_c = sum(1 for d in jogo if d % 2 == 0)
            jogos_com_score.append({
                "numero":      i,
                "dezenas":     jogo,
                "score":       round(score, 1),
                "nivel":       "ALTA" if score >= 68 else ("MEDIA" if score >= 52 else "BAIXA"),
                "soma":        soma,
                "pares":       pares_c,
                "alertas":     [],
                "explicacao":  f"soma={soma} pares={pares_c}",
            })
        jogos_com_score.sort(key=lambda x: -x["score"])
        ...
        dados_resultado = {
            ...
            "confianca":   estrategia.get("confianca", 65),
        }
        ...
        _ia_set(status="pronto", progresso=100,
                etapa="Concluido!", dados=dados_resultado, erro=None)
...
def api_analisar():
    ...
    threading.Thread(
        target=_rodar_analise, args=(pool_size, modo), daemon=True
    ).start()
...
def api_status():
    e = _get()
    return jsonify({
        "status": e["status"], "progresso": e["progresso"],
        "etapa": e["etapa"], "erro": e["erro"],
        "modo": e.get("modo", "basico"),
    })
...
def api_gerar_inteligente():
    ...
    if e["status"] != "pronto":
        return jsonify({
            "ok": False,
            "mensagem": "Execute a analise principal primeiro.",
        })
    _ia_set(status="carregando", progresso=0,
            etapa="Iniciando...", erro=None, dados=None)
    threading.Thread(
        target=_rodar_gerar_inteligente, daemon=True
    ).start()
...
def api_ai_resumo():
    ...
    return jsonify({"ok": True, "resumo": {
        ...
        "pesos_atuais": hib.get("pesos", {}),
        "acumulou":     (d.get("acumulacao") or {}).get("alerta", False),
        "premio_estimado": (d.get("acumulacao") or {}).get("valor_estimado", 0),
    }})
...
def api_ai_aplicar():
    ...
    try:
        ...
        if novos_pesos and isinstance(novos_pesos, dict):
            ...
            mudancas.append("Pesos atualizados via IA")
        pool_ia = resp_ia.get("pool_final")
        if pool_ia and isinstance(pool_ia, list) and len(pool_ia) >= 15:
            ...
            if len(pool_ia) == 18:
                ...
                if val["valida"]:
                    ...
                    mudancas.append(f"Pool IA aplicado: {pool_ia}")
...
def api_chat_historico():
    ...
    return jsonify({"ok": True, "historico": get_chat().historico[-30:]})
...
def api_node_status():
    return jsonify({
        "ok": False,
        "mensagem": "Node.js nao disponivel na nuvem. Puter.js funciona via CDN no frontend.",
    })
...
if __name__ == "__main__":
    ...
    print(f"LotofacilPro v4 - Nuvem - porta {porta}")
